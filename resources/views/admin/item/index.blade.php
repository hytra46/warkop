                        ...
                        <tbody>
                            @foreach ($items as $item)
                                <tr>
                                    ...
                                    <td>
                                        <span
                                            class="badge {{ $item->category->cat_name == 'Makanan' ? 'bg-warning' : ($item->category->cat_name == 'Minuman' ? 'bg-info' : 'bg-primary') }}"> {{ $item->category->cat_name }}
                                        </span>
                                    </td>
                                    <td>
                                        <span
                                            class="badge {{ $item->is_active == 1 ? 'bg-success' : 'bg-danger' }}"> {{ $item->is_active == 1 ? 'Aktif' : 'Tidak Aktif' }}
                                        </span>
                                    </td>
                                    <td>
                                        <a href="{{ route('items.edit', $item->id) }}" class="btn btn-sm btn-warning"><i class="bi bi-pencil"></i> Ubah</a>
